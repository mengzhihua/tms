...
import com.tms.tracking.service.TrackingService;
import com.tms.tracking.service.TrackingService.GpsReq;
import com.tms.tracking.service.TrackingService.HandleReq;
import java.util.*;
...
public class TrackingController {
    ...
    @PostMapping("/gps")
    public R<Integer> gps(@RequestBody GpsReq r) {
        return R.ok(service.gps(r));
    }

    @GetMapping("/vehicles")
    public R<List<Map<String, Object>>> vehicles() {
        return R.ok(service.vehicles());
    }

    @GetMapping("/events/page")
    public R<Page<TrackingEvent>> events(
    ...
    public R<Page<GeofenceAlert>> alerts(
            @RequestParam(defaultValue = "1") long current,
            @RequestParam(defaultValue = "20") long size,
    ...
    public R<Void> handle(@PathVariable Long id, @RequestBody HandleReq r) {
        service.handle(id, r);
        return R.ok();
    }

    @PostMapping("/simulate/{waybillId}")
    public R<Integer> simulate(
            @PathVariable Long waybillId, @RequestParam(defaultValue = "10") int steps) {
        return R.ok(service.simulate(waybillId, steps));
    }
}
