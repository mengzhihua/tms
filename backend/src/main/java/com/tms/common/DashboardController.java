...
public class DashboardController {
    ...
    public R<Map<String, Object>> summary() {
        ...
        for (TransportOrder o : orderMapper.selectList(null)) {
            os.put(o.getStatus(), os.getOrDefault(o.getStatus(), 0L) + 1);
        }
        for (Waybill w : waybillMapper.selectList(null)) {
            ws.put(w.getStatus(), ws.getOrDefault(w.getStatus(), 0L) + 1);
        }
        m.put("orderStatusCounts", os);
        m.put("waybillStatusCounts", ws);
        m.put(
                "inTransitVehicles",
                vehicleMapper.selectCount(new QueryWrapper<Vehicle>().eq("status", "IN_TRANSIT")));
        m.put(
                "idleVehicles",
                vehicleMapper.selectCount(new QueryWrapper<Vehicle>().eq("status", "IDLE")));
        m.put(
                "todayWaybills",
                waybillMapper.selectCount(
                        new QueryWrapper<Waybill>().ge("created_at", LocalDate.now().atStartOfDay())));
        m.put(
                "unhandledAlerts",
                alertMapper.selectCount(new QueryWrapper<GeofenceAlert>().eq("handled", false)));
        BigDecimal freight =
                waybillMapper.selectList(null).stream()
                        .map(Waybill::getFreightAmount)
        ...
        m.put(
                ...
                orderMapper.selectList(
                        new QueryWrapper<TransportOrder>()
                                .eq("status", "CREATED")
        ...
        m.put(
                ...
                eventMapper.selectList(
                        new QueryWrapper<TrackingEvent>().orderByDesc("event_time").last("LIMIT 20")));
        return R.ok(m);
    }
}
